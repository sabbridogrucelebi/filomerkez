...
Route::get('/debug-traffic-lights', function () {
    ...
    foreach ($endpoints as $url) {
        try {
            ...
            $results[] = "SUCCESS with {$url}: " . e(substr($response->getBody()->getContents(), 0, 100)) . "...";
        } catch (\Exception $e) {
            $results[] = "FAIL with {$url}: " . $e->getMessage();
        }
    }
    return "Endpoint Tests:<br><pre>" . implode("\n\n", $results) . "</pre>";
});
...
Route::get('/vehicle-tracking/reports', [LiveTrackingController::class, 'reports'])
    ->middleware(['auth', 'permission:vehicle_tracking.view'])
...
Route::get('/vehicle-tracking/reports/working', [\App\Http\Controllers\Api\V1\ReportsApiController::class, 'workingReportData'])
    ->middleware(['auth', 'permission:vehicle_tracking.view'])
...
Route::get('/vehicle-tracking/reports/speed-alarm', [\App\Http\Controllers\Api\V1\ReportsApiController::class, 'speedAlarmReportData'])
    ->middleware(['auth', 'permission:vehicle_tracking.view'])
